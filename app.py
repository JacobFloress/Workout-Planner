...
def create_workout_routine(experience, goals, frequency):
    db = get_db()
    days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
    week_tasks = []
    user_id = session['user_id']
    ...
    for day in days:
        encoded_daily_schedule = db.execute(f"select {day} from schedule where id = ?", [user_id]).fetchone()[
            0]  # get the daily schedule from the db
        decoded_daily_schedule = json.loads(encoded_daily_schedule)  # decode the schedule using loads function from json
        new_obj = config.Schedule(decoded_daily_schedule)  # create new Schedule class object based on the data structure we got from the db
        all_tasks = new_obj.get_free_time()  # get all tasks from the schedule using the get_all_tasks() method of the class
    ...
    match frequency:
        case 1:
            ...
            for task in week_tasks:
                for item in task[1]:
                    sep = item.split(" - ")
                    start_hour = int(sep[0][0:2])  # convert the start hour to int
                    start_min = int(sep[0][3:])  # convert the start min to int
                    end_hour = int(sep[1][0:2])  # convert the end hour to int
                    end_min = int(sep[1][3:])  # convert the end min to int
                    ...
                    if total_duration > 2 and start_hour > 8:
                        encoded_daily_schedule = db.execute(f"select {task[0]} from schedule where id = ?", [user_id]).fetchone()[
                            0]  # get the daily schedule from the db
                        decoded_daily_schedule = json.loads(encoded_daily_schedule)  # decode the schedule using loads function from json
                        new_obj = config.Schedule(decoded_daily_schedule)  # create new Schedule class object based on the data structure we got from the db
                        new_obj.add_task(title, f"{start_hour}:{start_min}", f"{start_hour + 2}:{start_min}")  # add the workout
        ...
        case 2:
            ...
            '''for item in week_tasks[1][1]: # Tuesday
                ...
                if total_duration > 2 and start_hour > 8:
                    encoded_daily_schedule = db.execute(f"select {week_tasks[1]} from schedule where id = ?", [user_id]).fetchone()[
                        0]  # get the daily schedule from the db
                    decoded_daily_schedule = json.loads(
                        encoded_daily_schedule)  # decode the schedule using loads function from json
                    new_obj = config.Schedule(
                        decoded_daily_schedule)  # create new Schedule class object based on the data structure we got from the db
                    new_obj.add_task(title[days_added], f"{start_hour}:{start_min}",
                                     f"{start_hour + 2}:{start_min}")  # add the workout
                    days_added = days_added + 1

                    print(new_obj.get_all_tasks())

                elif total_duration > 8:
                    encoded_daily_schedule = db.execute(f"select {week_tasks[1][0]} from schedule where id = ?", [user_id]).fetchone()[
                        0]  # get the daily schedule from the db
                    decoded_daily_schedule = json.loads(encoded_daily_schedule)  # decode the schedule using loads function from json
                    new_obj = config.Schedule(decoded_daily_schedule)  # create new Schedule class object based on the data structure we got from the db
                    # if the digits are less than 10, put 0 before the digit
                    if int(start_hour) < 10:
                        start_hour = f"0{start_hour + 8}"
                    if start_min < 10:
                        start_min = f"0{start_min}"
                    if int(start_hour) < 10:
                        end_hour = f"0{int(start_hour) + 2}"
                    if end_min < 10:
                        end_min = f"0{start_min}"

                    new_obj.add_task(title[days_added], f"{start_hour}:{start_min}",
                                     f"{end_hour}:{end_min}")  # add the workout
                    days_added = days_added + 1

                    print(new_obj.get_all_tasks())

            for item in week_tasks[3]:  # Thursday
                sep = item[1].split(" - ")
                start_hour = int(sep[0][0:2])  # convert the start hour to int
                start_min = int(sep[0][3:])  # convert the start min to int
                end_hour = int(sep[1][0:2])  # convert the end hour to int
                ...
                if total_duration > 2 and start_hour > 8:
                    encoded_daily_schedule = \
                    db.execute(f"select {week_tasks[1]} from schedule where id = ?", [user_id]).fetchone()[
                        0]  # get the daily schedule from the db
                    decoded_daily_schedule = json.loads(
                        encoded_daily_schedule)  # decode the schedule using loads function from json
                    new_obj = config.Schedule(
            ...
            '''days_added = 0
            for task in range(len(week_tasks)):
                for item in week_tasks[task + days_added]:
                    sep = item.split(" - ")
                    start_hour = int(sep[0][0:2])  # convert the start hour to int
                    start_min = int(sep[0][3:])  # convert the start min to int
                    end_hour = int(sep[1][0:2])  # convert the end hour to int
                    ...
                    if total_duration > 2 and start_hour > 8:
                        ...
                        decoded_daily_schedule = json.loads(encoded_daily_schedule)  # decode the schedule using loads function from json
                        new_obj = config.Schedule(decoded_daily_schedule)  # create new Schedule class object based on the data structure we got from the db
                        new_obj.add_task(title[days_added], f"{start_hour}:{start_min}", f"{start_hour + 2}:{start_min}")  # add the workout
                        days_added = days_added + 1

                        print(new_obj.get_all_tasks())
                    elif total_duration > 8 and start_hour > 8:
                        encoded_daily_schedule = db.execute(f"select {task[0]} from schedule where id = ?", [user_id]).fetchone()[0]  # get the daily schedule from the db
                        decoded_daily_schedule = json.loads(encoded_daily_schedule)  # decode the schedule using loads function from json
                        new_obj = config.Schedule(decoded_daily_schedule)  # create new Schedule class object based on the data structure we got from the db
                        ...
                        new_obj.add_task(title[days_added], f"{start_hour}:{start_min}", f"{end_hour}:{end_min}")  # add the workout
                        days_added = days_added + 1
...
def workout():
    if session['user_id']:
        ...
        create_workout_routine(exp_goals[0][0], exp_goals[0][1], 2)
...
def current_workout():
    ...
    return render_template('workout_button.html', current_workouts=current_workouts)
...
def save_current_workout():
    ...
    # Redirect back to the current workout page
    return render_template('workout_button.html', current_workouts=updated_workouts)
