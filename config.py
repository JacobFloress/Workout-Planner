class Schedule:
    ...
    def get_free_time(self):
        """ Returns free time slots in the schedule """
        ...
        for hour in range(24):
            for minute in range(60):
                if self.schedule[str(hour)][minute] is None:
                    if free_start is None:
                        free_start = (hour, minute)
                else:
                    # Ensure there are no single-minute gaps
                    if free_start is not None:
                        end_hour, end_minute = hour, minute
                        if (end_hour * 60 + end_minute) - (free_start[0] * 60 + free_start[1]) > 1:
                            time_periods.append(
                                f"{free_start[0]:02}:{free_start[1]:02} - {end_hour:02}:{end_minute:02}"
                            )
                        free_start = None

        # Handle any remaining free time till the end of the day
        if free_start is not None:
            time_periods.append(f"{free_start[0]:02}:{free_start[1]:02} - 23:59")
...
print(new_obj.get_all_tasks())
print("==========")
print(new_obj.get_free_time())
new_obj.add_task("SWD", "14:00", "15:00")
print(new_obj.get_all_tasks())
print("==========")
print(new_obj.get_free_time())'''
